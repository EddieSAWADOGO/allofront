...
document.addEventListener("DOMContentLoaded", () => {
        ...
        profileModalContent.innerHTML = `
            <div class="profile-top">
                <div class="profile-info-left">
                    ...
                    <p>Démarcheur certifié • ${broker.city}</p>
                </div>
                <img src="${broker.avatar}" alt="${broker.firstname}" class="profile-avatar-right">
            </div>

            <div class="profile-pro-card-area">
                <p class="form-label" style="font-size: 0.7rem; margin-bottom: 5px;">Carte Professionnelle</p>
                <img src="${broker.proCard}" alt="Carte Pro" class="profile-pro-card-img">
            </div>

            <div class="profile-actions-grid">
                <a href="tel:+226${broker.phone}" class="btn btn-secondary" style="gap: 6px;">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
                    Appeler
                </a>
                <a href="https://wa.me/226${broker.phone}" target="_blank" class="btn btn-primary" style="background-color: #25D366; border-color: #25D366; gap: 6px;">
            ...
            <button class="btn btn-secondary btn-block btn-share-profile" data-id="${broker.id}">
                <svg style="margin-right:8px;" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg>
                Partager le profil
            </button>
        `;
    ...
    // --- Ticket Form Logic (Index Page) ---
    const requestTicketForm = document.getElementById("request-ticket-form");
    if (requestTicketForm) {
        const prestationRent = document.getElementById("prestation-rent");
        const prestationBuy = document.getElementById("prestation-buy");
        const bienLand = document.getElementById("bien-land");
        const bienHouse = document.getElementById("bien-house");
        ...
        function updateBienFields() {
            if (prestationRent.checked) {
                bienLand.disabled = true;
                bienLand.checked = false;
                bienHouse.checked = true;
                if (landHint) landHint.style.display = "block";
                if (labelLand) {
                    labelLand.style.opacity = "0.5";
                    labelLand.style.cursor = "not-allowed";
                }
            } else {
                bienLand.disabled = false;
                if (landHint) landHint.style.display = "none";
                if (labelLand) {
                    labelLand.style.opacity = "1";
                    labelLand.style.cursor = "pointer";
                }
            }
        }

        prestationRent.addEventListener("change", updateBienFields);
        prestationBuy.addEventListener("change", updateBienFields);
        updateBienFields();

        requestTicketForm.addEventListener("submit", (e) => {
            e.preventDefault();
            ...
            if (localStorage.getItem(lastSubmissionKey) === todayStr) {
                showNotification("Demande Bloquée", "Une seule demande par jour autorisée pour ce numéro.", "error");
                return;
            }

            localStorage.setItem(lastSubmissionKey, todayStr);
            showNotification("Demande Envoyée !", "Votre demande a été transmise aux démarcheurs.", "success");
            requestTicketForm.reset();
            updateBienFields();
        });
    }
